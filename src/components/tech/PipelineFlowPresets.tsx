...
import { Layers, Globe, Server, Terminal, Lock } from 'lucide-react';

export type FlowDomainId = 'all' | 'web' | 'backend' | 'devops' | 'cybersecurity';
          ...
          return (
            <button
              ...
              className={`p-3 rounded-2xl border-2 border-[#0f172a] text-left transition-all duration-200 cursor-pointer select-none flex flex-col justify-between ${
                isActive
                  ? 'bg-[#ffc53d] shadow-[4px_4px_0px_#0f172a] translate-x-0.5 translate-y-0.5'
                  : 'bg-[#fffffb] shadow-[2px_2px_0px_#0f172a] hover:bg-[#fff3df] hover:shadow-[4px_4px_0px_#0f172a]'
              }`}
            >
              <div className="flex items-center justify-between">
                <div
                  className={`w-7 h-7 rounded-lg border border-[#0f172a] flex items-center justify-center ${
                    isActive ? 'bg-[#0f172a] text-[#ffc53d]' : 'bg-[#fdeed3] text-[#0f172a]'
                  }`}
                >
                  <Icon className="w-4 h-4" />
                </div>

                <span className="text-[9px] font-mono font-bold uppercase px-1.5 py-0.5 rounded border bg-[#fdeed3] text-[#0f172a] border-[#0f172a]/30">
                  {domain.tag}
                </span>
              </div>
