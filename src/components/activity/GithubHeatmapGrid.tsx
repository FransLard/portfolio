...
const getBeachLevelColor = (level: number): string => {
  switch (level) {
    ...
    case 0:
    default:
      return 'bg-[#fdeed3] border-[#ecd0a0] hover:border-[#cbd5e1]';
  }
};
...
export const GithubHeatmapGrid: React.FC<GithubHeatmapGridProps> = ({ contributions, loading }) => {
  ...
  return (
    <div ref={containerRef} className="relative w-full rounded-2xl bg-[#fff3df]/70 border-2 border-[#0f172a] p-3.5 sm:p-6 shadow-[3px_3px_0px_#0f172a]">
      <AnimatePresence>
        {hoveredCell && (
          <motion.div
      ...
      <div className="heatmap-scroll-wrapper overflow-x-auto pb-2 select-none no-scrollbar sm:scrollbar-thin">
        <div className="min-w-[760px] flex flex-col">
          ...
          <div className="flex text-[11px] font-mono font-bold text-[#8a5a24] mb-2.5 pl-8">
            {weeks.map((_, colIdx) => {
          ...
          <div className="flex items-start">
            ...
            <div className="flex flex-col justify-between h-[116px] pr-3 text-[10px] font-mono font-bold text-[#8a5a24]">
              <span>Sen</span>
              <span>Rab</span>
              <span>Jum</span>
            </div>
      ...
      <div className="mt-3.5 sm:mt-5 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs text-[#64748b] font-mono pt-3 border-t border-[#ecd7b0]">
        <span className="text-[10px] sm:text-[11px] text-[#8a5a24] font-medium">
          Data sinkron otomatis dengan riwayat git GitHub
        </span>

        <div className="flex items-center gap-1.5 self-end sm:self-auto">
          <span className="text-[10px] sm:text-[11px] mr-1 text-[#8a5a24]">Sedikit</span>
          {[0, 1, 2, 3, 4].map((level) => (
            <div
              key={level}
          ...
          <span className="text-[10px] sm:text-[11px] ml-1 text-[#8a5a24]">Banyak</span>
        </div>
      </div>
    </div>
  );
