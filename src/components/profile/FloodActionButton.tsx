      ...
      <AnimatePresence mode="wait" initial={false}>
        {floodState === 'pristine' && (
          <motion.button
            ...
            className="flex items-center gap-2.5 px-4 py-2.5 rounded-2xl bg-[#0e9384] hover:bg-[#0b6e63] text-white text-xs font-mono font-bold shadow-lg shadow-[#0e9384]/20 transition-all cursor-pointer hover:scale-105 active:scale-95 group"
        ...
        {floodState === 'rebuilding' && (
          <motion.div
            ...
            exit={{ opacity: 0, scale: 0.96 }}
            transition={{ duration: 0.18 }}
            className="flex items-center gap-3 px-4 py-2.5 rounded-2xl bg-[#fffffb] border border-[#ecd7b0] shadow-md"
          >
